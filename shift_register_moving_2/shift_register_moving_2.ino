...
// 1 2 3 4
// 5 6 7 8
byte patterns[] = { B00000001, B00000010, B00000100, B00001000 };
...
void loop() {
  for (int i = 0; i < NUM_PATTERNS; i++) {
    ...
    // Turn on the LED across from the first by shifting the current pattern byte by 4
    byte nextPattern = patterns[i] << 4;
    writeToShiftRegister(DATA_PIN, CLOCK_PIN, nextPattern);
    digitalWrite(LATCH_PIN, HIGH);
    digitalWrite(LATCH_PIN, LOW);
    delay(DELAY);
  }
